...
export interface VariantNewFormValues {
  sku: string;
  price: number | "";
  stock: number | "";
  originId: number | "";
...
export default function NewProductVariantPage() {
  ...
  const { createProductVariant } = useProductVariantAdminHook(productId);
  ...
  const formik = useFormik<VariantNewFormValues>({
    initialValues: {
      sku: "",
      price: "",
      stock: 0,
      ...
      weightGrams: "",
      size: "",
      beadCount: isIndividual ? "" : 108,
      material: isIndividual ? "" : "Pure Silk Thread",
      imageFile: null,
    },
    validationSchema,
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        toast.success(`Variant "${values.sku}" created successfully!`);
  ...
  const isBusy =
    formik.isSubmitting || isUploading || createProductVariant.isPending;
      ...
      <div className="space-y-6 max-w-4xl mx-auto p-4">
        ...
        <Card className="border-red-200 bg-red-50/40 p-8 text-center shadow-xs">
          ...
          <p className="text-xs text-red-700 mt-1">
            Cannot add variant to a non-existent product ID #{productId}.
          </p>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6 pb-16 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-xs">
        ...
        <span className="text-muted-foreground/60">/</span>
        <span className="font-bold text-[#713f12]">Add New Variant</span>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs sm:text-sm text-[#5c3a1e]/80 max-w-xl">
              Configure inventory SKU, exact bead dimensions (mm), origin, and
              selling price.
            </p>
          </div>
        ...
        <Card className="border-amber-900/15 bg-white shadow-xs">
          <CardHeader className="border-b border-amber-900/10 pb-4 bg-amber-50/20">
            ...
            <CardDescription className="text-xs text-[#5c3a1e]/70">
              Set unique SKU identifier, price in NPR, and initial stock count.
            </CardDescription>
          </CardHeader>
          ...
          >
            {isBusy ? (
              <>
                ...
                <span>
                  {isUploading ? "Uploading Image..." : "Saving Variant..."}
                </span>
              </>
            ) : (
              <>
